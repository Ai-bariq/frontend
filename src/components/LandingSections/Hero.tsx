...
function TrustedLogos({ label }: { label: string }) {
  return (
    <div className="mt-16 w-full sm:mt-20">
      <p className="mb-5 text-center text-xs font-medium text-slate-400 sm:text-sm">{label}</p>
...
function TypewriterWord({ words }: { words: string[] }) {
  ...
  return (
    <span className="inline-flex min-w-[72px] items-center justify-center rounded-2xl bg-amber-100 px-4 py-2 leading-none text-amber-800 shadow-sm">
      <span className="inline-block min-h-[1em] whitespace-nowrap">{displayText || ' '}</span>
...
export default function HeroSection() {
  ...
  return (
    <section dir={dir} className="relative overflow-hidden border-t border-slate-200 bg-white" id="hero">
        ...
        <div className="flex min-h-[92vh] flex-col items-center justify-center py-14 sm:py-16 lg:py-20">
          <div className="mb-8 sm:mb-10">
            <div className="flex h-24 w-24 items-center justify-center rounded-full border border-teal-100 bg-white/70 shadow-[0_20px_60px_rgba(20,184,166,0.16)] sm:h-28 sm:w-28">
              <img src={avatar} alt="AI assistant" className="h-20 w-20 rounded-full object-cover sm:h-24 sm:w-24" />
            </div>
          </div>
          ...
          <div className="mx-auto max-w-5xl text-center">
            ...
            <div className="mt-6 flex flex-col items-center justify-center gap-3 text-xl font-bold sm:mt-7 sm:flex-row sm:gap-4 sm:text-2xl lg:text-[2rem]">
              <span className="text-teal-600">{t.hero.taglinePrefix}</span>
              <TypewriterWord words={t.hero.words} />
              <span className="text-teal-600">{t.hero.taglineSuffix}</span>
            </div>

            <div className="mt-10 flex flex-col items-center gap-3 sm:mt-12">
              <p className="text-lg font-bold text-teal-600 sm:text-xl">{t.hero.tryNow}</p>
            ...
            <div className="mx-auto mt-10 w-full max-w-[770px] rounded-2xl bg-gradient-to-br from-teal-600 via-teal-500 to-emerald-600 p-5 shadow-[0_18px_50px_rgba(13,148,136,0.22)] sm:mt-12 sm:p-6 lg:p-7">
              <div>
                <div className="flex items-center gap-3 rounded-2xl border border-slate-200 bg-white px-4 py-4 shadow-sm sm:px-6">
                  <img src={maps} alt="Google Maps" className="h-6 w-6 shrink-0 object-contain" />
                  <input
                    type="text"
                    placeholder={t.hero.searchPlaceholder}
                    className="flex-1 bg-transparent text-sm text-black placeholder:text-slate-400 focus:outline-none sm:text-base"
                    style={{ textAlign: dir === 'rtl' ? 'right' : 'left' }}
                  />
                </div>
              </div>
              <p className="mt-4 text-center text-xs text-slate-100 sm:text-sm">{t.hero.note}</p>
            </div>
          </div>

          <TrustedLogos label={t.hero.trusted} />
        </div>
      </div>
    </section>
  )
