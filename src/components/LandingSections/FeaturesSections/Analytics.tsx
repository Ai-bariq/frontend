...
const POINT_ICONS = [Activity, BarChart3, TriangleAlert, FileText]

const ANALYTICS_BARS = [28, 33, 25, 30, 27, 32, 23, 29, 20, 25, 18, 23] as const
const ANALYTICS_SCORE = 75
const ANALYTICS_STATS = [
  { id: 'reviews', value: '24', trend: '+12%' },
  { id: 'rating', value: '4.5', trend: '+0.3' },
  { id: 'sentiment', value: '82%', trend: '+5%' },
]
...
const STYLES = {
  analytics: {
    ...
    layout: 'mx-auto mt-8 grid max-w-6xl grid-cols-1 items-center gap-8 lg:grid-cols-2 lg:gap-12',
    reveal: {
    ...
    title: 'mt-3 text-3xl font-bold tracking-tight text-slate-900 sm:text-4xl lg:text-[2.65rem]',
    ...
    dashboardOuter: 'relative mx-auto w-full max-w-[400px] sm:max-w-[430px] lg:max-w-[470px] min-h-[300px]',
    ...
    dashboardTopBar: 'flex items-center justify-between border-b border-slate-200 bg-[#f4f8f8] px-3 py-2 sm:px-3.5 sm:py-2.5',
    ...
    periodPillBase: 'rounded-md px-2 py-1 text-[10px] font-medium leading-none sm:px-2.5 sm:text-[11px]',
    ...
    dashboardHeroText: 'flex w-full flex-col items-end text-right leading-none',
    scoreRingWrap: 'relative flex h-12 w-12 shrink-0 items-center justify-center rounded-full sm:h-14 sm:w-14',
    scoreRing: 'absolute inset-0 rounded-full',
    scoreRingCenter: 'absolute inset-[5px] rounded-full bg-white sm:inset-[6px]',
    scoreValue: 'relative z-10 text-sm font-bold text-slate-900 sm:text-lg',
    scoreGroup: 'flex items-center justify-end gap-2',
    dashboardHeroTitle: 'text-sm font-bold text-slate-900 sm:text-base',
    dashboardHeroSub: 'mt-0.5 w-full text-right text-[11px] font-bold text-emerald-600 sm:text-xs',
    statsGrid: 'mt-4 grid grid-cols-3 gap-2',
    statCard: 'rounded-xl bg-[#f5f7f9] px-2 py-2.5 text-center sm:px-2.5 sm:py-3',
    ...
    chartTitle: 'text-[11px] font-bold text-slate-500 sm:text-xs',
    chartBars: 'mt-2 flex h-12 flex-row-reverse items-end justify-between gap-1',
    chartBar: 'flex-1 rounded-t-md bg-gradient-to-t from-[#40c9b3] to-[#5fd6c3]',
  },
} as const
...
export default function AnalyticsSection() {
  ...
  const textCol = isRTL ? STYLES.analytics.reveal.rightHidden : STYLES.analytics.reveal.leftHidden
  const dashCol = isRTL ? STYLES.analytics.reveal.leftHidden : STYLES.analytics.reveal.rightHidden

  const dash = t.features.analytics.dashboard

  return (
    <section ref={ref} dir={dir} className={STYLES.analytics.wrapper}>
      <div className={`${STYLES.analytics.container} mx-auto w-full max-w-7xl px-4 sm:px-6 lg:px-8`}>
        <div className={STYLES.analytics.layout}>
          <div className={[textAlign, STYLES.analytics.reveal.base, isVisible ? STYLES.analytics.reveal.visible : textCol].join(' ')}>
            <div className={STYLES.analytics.badge}>
              <BarChart3 className={STYLES.analytics.badgeIcon} />
              {t.features.analytics.badge}
            </div>
            <h2 className={STYLES.analytics.title}>{t.features.analytics.sectionTitle}</h2>
            <p className={STYLES.analytics.subtitle}>{t.features.analytics.sectionSubtitle}</p>
            <div className={STYLES.analytics.pointsList}>
              {t.features.analytics.points.map((point, i) => {
                const Icon = POINT_ICONS[i]
                return (
                  <div key={point.id} className={STYLES.analytics.pointRow}>
          ...
          <div className={[STYLES.analytics.reveal.base, isVisible ? STYLES.analytics.reveal.visible : dashCol].join(' ')}>
              ...
              <div className={STYLES.analytics.dashboardCard}>
                <div className={STYLES.analytics.dashboardTopBar}>
                  ...
                  <div className={STYLES.analytics.dashboardTopTabs}>
                    {dash.periods.map((p, i) => (
                      <span key={p} className={`${STYLES.analytics.periodPillBase} ${i === 0 ? STYLES.analytics.periodPillActive : STYLES.analytics.periodPillMuted}`}>{p}</span>
                    ))}
                  </div>
                </div>
                ...
                <div className={STYLES.analytics.dashboardBody}>
                  <div className={STYLES.analytics.dashboardHero}>
                    <div className={STYLES.analytics.scoreGroup}>
                      <div className={STYLES.analytics.scoreRingWrap}>
                        <div className={STYLES.analytics.scoreRing} style={{ background: `conic-gradient(#10b981 ${ANALYTICS_SCORE * 3.6}deg, #e5eef0 0deg)` }} />
                        <div className={STYLES.analytics.scoreRingCenter} />
                        <span className={STYLES.analytics.scoreValue}>{ANALYTICS_SCORE}</span>
                      </div>
                      <div className={STYLES.analytics.dashboardHeroText}>
                        <p className={STYLES.analytics.dashboardHeroTitle}>{dash.scoreLabel}</p>
                        <p className={STYLES.analytics.dashboardHeroSub}>{dash.scoreStatus}</p>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className={STYLES.analytics.statsGrid}>
                    {ANALYTICS_STATS.map((stat, i) => (
                      <div key={stat.id} className={STYLES.analytics.statCard}>
                        ...
                        <div className={STYLES.analytics.statLabel}>{dash.stats[i]?.label}</div>
                  ...
                  <div className={STYLES.analytics.chartCard}>
                    <div className={`${STYLES.analytics.chartTitle} ${textAlign}`}>{dash.chartTitle}</div>
                    <div className={STYLES.analytics.chartBars}>
                      {ANALYTICS_BARS.map((height, index) => (
                        <div key={`${height}-${index}`} className={STYLES.analytics.chartBar} style={{ height: `${height}px` }} />
                      ))}
                    </div>
                  </div>
                </div>
