...
function EditedResponsesPage() {
  ...
  // No extra deps — fetches once on mount.
  // React StrictMode double-invoke in dev is acceptable; do not hack around it.
  useEffect(() => {
    let cancelled = false
    const load = async () => {
      setIsLoading(true)
    ...
    const base = q
      ? replies.filter((r) => {
          ...
          return (
            ...
            (r.aiText ?? '').toLowerCase().includes(q) ||
            (r.editedText ?? '').toLowerCase().includes(q)
          )
        })
      : replies
    ...
    <section dir={dir} className="min-h-screen bg-slate-50 p-4 md:p-6">
      <div className="mx-auto w-[90%] max-w-none space-y-6">
        ...
        <div className="w-full overflow-x-auto rounded-2xl border border-slate-200 bg-white">
          <table className={`w-full min-w-[1100px] table-fixed ${textAlign} text-base`}>
            ...
            <tbody>
              {isLoading ? (
                ...
                <tr>
                  <td colSpan={6} className="px-4 py-10 text-center text-sm text-red-500">{error}</td>
              </tr>
              ) : filtered.length === 0 ? (
                <tr>
                ...
                filtered.map((item) => {
                  ...
                  const aiDisplay =
                    ...
                    item.previousPublishedText?.trim() ||
                    null
                  const editedDisplay = item.editedText?.trim() || item.finalText?.trim() || null
                  ...
                  return (
                    <tr key={item._id} className="border-t border-slate-100 align-top text-sm text-slate-700">
                      ...
                      >
                        {aiDisplay
                          ? <p className={`break-words ${isExpanded ? 'whitespace-pre-wrap' : 'line-clamp-3'}`}>{aiDisplay}</p>
                          : <span className="italic text-slate-400">{p.aiNotSaved}</span>
                        }
                      </td>
                      <td
